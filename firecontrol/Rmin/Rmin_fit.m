        ...
        for vm = vm_idx
            for vt = vt_idx
                ...
                while pi_idx <= 180/qsj_interval+1
                    ...
                    R = (Ra + Rb)/2;
                    Rmin(min_idx) = R;                  
                    qsj = deg2rad(qsj_interval * min_idx);
                    %disp(aero_idx);
                    min_idx = min_idx + 1;
                end
                Rmin(end:-1:180/qsj_interval+2) = Rmin(1:180/qsj_interval);              
                key{vm_idx==vm, vt_idx==vt, i+4} = fit_condition(vm/340,vt/340,ym/1000,yt/1000);
                rvalue{vm_idx==vm, vt_idx==vt, i+4} = Rmin;
                disp(['Rmin vm ' num2str(vm) ' vt ' num2str(vt) ' hm ' num2str(ym) ' ht ' num2str(yt) ' rvalue ' num2str(Rmin./1000)]);
            end
        end
    end
    ym_i = ym_i + 1;
    save(['min_', num2str(ym_i), '.mat'], 'key', 'rvalue', 'qvalue');
    key = cell(length(vm_idx), length(vt_idx), 7);
    rvalue = cell(length(vm_idx), length(vt_idx), 7);
end
%% fit
...
coe_min = cal_coe('min.mat', 15, qsj_num, vm_idx, vt_idx, hm_idx);
save('coe_min.mat', 'coe_min');